...
| Issue | Bug | Demo |
|---|---|---|
| #3 | Can't create `dark-mode` in development because it exists in production (409) | **Debugging:** investigate, then fix |
...
```
/piv-investigate-issue 3      # parallel exploration, 5 Whys, RCA to docs/issues/issue-3.md + a comment on #3
/piv-implement-issue 3        # fresh session: drift-check the RCA, fix, regression test, docs/issues/fix-report-3.md
```
...
```
Then, in Claude Code: `/piv-investigate-issue 6`. When that session stops, `baton.sh` sees
`docs/issues/issue-6.md` with no fix report and starts `/piv-implement-issue 6` in a fresh `claude -p`. Show
`docs/issues/.baton-6.log` filling up. Point at: nobody asked for the second step. **Remove
